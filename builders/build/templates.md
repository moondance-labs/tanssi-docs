...
Besides Cumulus, a ContanerChain must implement the Tanssi modules to support the protocol and benefit safely from the block production as a service:
...
If you don't include these modules in the ContainerChain's runtime, there won't be a method to confirm that the blocks are being generated by trustworthy collators chosen by the Tanssi orchestrator. This could create a vulnerability for malicious actors to exploit and compromise the ContainerChain.
...
If the runtime is set up as a solo-chain, check on the official [Cumulus template](https://github.com/paritytech/polkadot-sdk/tree/master/cumulus/parachain-template){target=_blank} or in any of the templates available in the [Tanssi repository](https://github.com/moondance-labs/tanssi){target=_blank} for a reference setup.
