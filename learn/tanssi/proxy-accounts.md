---
title: Setup and Manage Proxy Accounts
...
Proxy accounts can be set up to perform a limited number of actions on behalf of primary accounts and are helpful for keeping the underlying accounts safe. Your proxy account can act as a "hot wallet" to perform regular maintenance duties on behalf of your "cold wallet" block producer account. For added safety, you can regularly rotate the proxy account. 
...
- **`Staking`** - allows the proxy account to perform staking-related transactions, such as block producer and `authorMapping()` functions 
...
You also have the option of creating a proxy of type **Any** which grants the proxy account full and unrestricted control over the primary account. This means that the proxy account can transfer funds, and perform any arbitrary action. The following demo will showcase configuring a **Balances** proxy, which is more restrictive than a proxy of type **Any**, but allows for transferring funds, which makes it perfect for demonstration purposes. After configuring your **balances** proxy you can try transferring funds from the primary account via proxy. 
