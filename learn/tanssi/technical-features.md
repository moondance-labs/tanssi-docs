...
The protocol also allows Appchains to select and connect to an external security provider (like [Symbiotic](https://symbiotic.fi/){target=\_blank}), ensuring Ethereum-grade security right from the start.
...
- **Node operators** - who register their hardware in Tanssi, offering their block production services to get rewards 
- **Appchains** - which require the services to execute transactions and keep the network's liveness

The protocol assigns a subset of sequencers to provide services to an appchain, rotating them after a period of time. The sequencers can serve any appchain, regardless of the custom logic they have implemented. On the other hand, appchains deployed through Tanssi can customize their runtime as much as they need to fit their use case and upgrade the logic at any moment in a forkless fashion without worrying about the sequencer's setup.
...
Once the sequencer set that will participate in the next session providing services is known, Tanssi shuffles the list and assigns them to provide block production services to the active Tanssi Appchains.
...
As a sequencer node assigned to a Tanssi appchain includes built-in Tanssi node functionality, it is technically feasible to read the state from the Tanssi network.
...
Once the block is completed with the Tanssi appchain transactions, it will be proposed as a candidate and handed over to the Tanssi chain, where the security provider's operators will ensure that the included state proofs match the state proofs from the latest state of Tanssi (preventing unauthorized block production) and that the transactions produced valid state transitions. Having verified the work of the sequencer, the Tanssi networks will finalize the proposed block, including its candidate receipt in a block.
