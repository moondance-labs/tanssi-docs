...
As redes implementadas através do Tanssi recebem muitos [benefícios](/learn/tanssi/overview/#what-tanssi-provides){target=\_blank}, como produção de blocos como um serviço, recuperabilidade de dados como um serviço e segurança através de [provedores de segurança externos](/learn/tanssi/external-security-providers/){target=\_blank}, como o [Symbiotic](https://symbiotic.fi/){target=\_blank} no Ethereum.

Além disso, como as redes com tecnologia Tanssi são baseadas em uma [pilha de tecnologia modular](/learn/framework/){target=\_blank}, elas se beneficiam de vantagens exclusivas quando as personalizações são necessárias em um nível de tempo de execução. Esta [modularidade](/learn/framework/modules/){target=\_blank} permite que os desenvolvedores adicionem funcionalidades diretamente no tempo de execução ou estendam as capacidades da própria EVM através de contratos pré-compilados.

Por exemplo, o Tanssi fornece um [modelo](/learn/decentralized-networks/included-templates#baseline-evm-template){target=\_blank} pronto para uso que inclui módulos [Frontier](https://github.com/polkadot-evm/frontier){target=\_blank}, permitindo a implementação sem esforço de redes compatíveis com EVM, semelhante ao [Moonbeam](https://moonbeam.network){target=\_blank}.
