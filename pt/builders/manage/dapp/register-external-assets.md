...
1. **Painel de Ativos Registrados** - esta seção irá agrupar e apresentar a você todos os ativos registrados que sua rede já tem disponíveis
1. **Lista de Ativos Registrados** - os ativos externos já registrados serão exibidos nesta seção, juntamente com suas informações associadas, como nome do ativo, símbolo, ID, oferta total e ID da rede onde é nativo
1. **Registro de Ativos** - esta opção permite que você selecione outros ativos externos disponíveis e registre-os. A [seção a seguir](#register-external-asset) explica como fazê-lo
...
Para fazer isso, clique em **Registro de Ativos** e depois:
