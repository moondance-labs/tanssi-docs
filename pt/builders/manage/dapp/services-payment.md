...
Como apresentado no artigo [Produção de Blocos como Serviço](/learn/tanssi/network-services/block-production/#block-production-fees){target=\_
blank}, existem dois custos principais associados que o governador da rede deve cobrir:

- **Atribuição de sequenciadores** - para a atribuição de sequenciadores pelo protocolo Tanssi, que acontece uma vez por sessão
- **Produção de blocos** - para cada bloco que é produzido em nome da rede
...
## Recarregando os Créditos da Sua Rede {: #topping-up }
...
Você pode verificar o saldo atual da sua rede e recarregá-lo usando o Tanssi dApp. Para fazer isso, acesse o [Tanssi dApp](https://apps.tanssi.network/){target=\_blank} e conecte a conta de registro da rede ao dApp. O site exibirá um cartão mostrando o status da sua rede. Este cartão inclui a previsão de atividade projetada abaixo da seção **Produção de Blocos** e o botão **Recarregar**.

![Botão de ação de recarga no dApp](/images/builders/manage/dapp/services-payment/services-payment-1.webp)

Clicar no botão **Recarregar** exibe uma barra lateral onde as seguintes informações podem ser vistas:

- **Saldo atual** - o saldo atual alocado para o serviço de produção de blocos da rede
- **Saldo disponível** - o saldo disponível na conta de registro da rede, que está conectada ao dApp
- **Custo atual** - custo atual por atribuição de sequenciador e custo por bloco
- **Previsão projetada** - o dia estimado em que a rede ficará sem fundos e deixará de ser atendida
