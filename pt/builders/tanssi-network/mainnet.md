...
Esta página de referência rápida oferece todos os elementos essenciais que você precisa para começar na Rede Tanssi.

## TANSSI Token {: #tanssi-token }

Os tokens {{ networks.mainnet.token_symbol }} servem como a moeda nativa dentro da Tanssi Network. Para iniciar a implantação de uma rede na Tanssi, você precisará obter tokens {{ networks.mainnet.token_symbol }}.

## Endpoints de Rede {: #tanssi-network-endpoints }

A Tanssi MainNet possui dois tipos de endpoints disponíveis para os usuários se conectarem: um para HTTPS e outro para WSS.
